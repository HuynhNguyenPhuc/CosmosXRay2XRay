"""Trainer entrypoint for Cosmos-Predict 2.5 multiview X-ray synthesis."""
...
from predict2_5.callback import MultiviewTensorBoardCallback, MultiviewWanDBCallback
...
class MultiviewTrainingConfig:
    """Configuration for multiview X-ray training."""

    # ── Paths ──
    data_dirs: str = ""  # Comma-separated NIfTI source directories (for cache building)
    cache_dir: str = "./cache"
    output_dir: str = "outputs/predict2.5_multiview"
    ...
    # ── Multiview ──
    views_per_batch: int = 1  # Random sampling each batch: 1-7 views (1 safest for most GPUs)
    view_loss_weights: bool = True
    prefer_difficult_views: bool = True


def get_callbacks(config: MultiviewTrainingConfig) -> List:
    callbacks = [
    ...
    if config.logger_type == "wandb":
        callbacks.append(MultiviewWanDBCallback())
    else:
        callbacks.append(MultiviewTensorBoardCallback())
    if config.enable_ema:
        callbacks.append(EMAMonitorCallback())
    return callbacks


def _build_logger(config: "MultiviewTrainingConfig"):
    if config.logger_type == "wandb":
        return WandbLogger(
            project=config.wandb_project,
            name=config.wandb_name or f"predict2.5_mv_{config.model_size}",
...
def get_strategy(config: MultiviewTrainingConfig):
...
def train(config: MultiviewTrainingConfig):
    """Run training."""
    ...
    model_kwargs = {
        ...
        "view_loss_weights": config.view_loss_weights,
        "prefer_difficult_views": config.prefer_difficult_views,
    }
    ...
    model = CosmosXRay2XRayMultiview(**model_kwargs)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Cosmos-Predict 2.5 Multiview X-ray Training")
    parser.add_argument("--data-dirs", type=str, default="", help="Comma-separated NIfTI source dirs")
    parser.add_argument("--cache-dir", type=str, default="./cache")
    parser.add_argument("--output-dir", type=str, default="outputs/predict2.5_multiview")
    parser.add_argument("--checkpoint-path", type=str, default=None)
    parser.add_argument("--tokenizer-path", type=str, default=None)
    parser.add_argument("--model-size", type=str, default="2B")
    parser.add_argument("--batch-size", type=int, default=1)
    parser.add_argument("--accum", type=int, default=1, help="Number of batches to accumulate for gradient update")
    parser.add_argument("--num-gpus", type=int, default=1)
    parser.add_argument("--max-epochs", type=int, default=200)
    parser.add_argument("--max-iters", type=int, default=100000)
    parser.add_argument("--learning-rate", type=float, default=2 ** (-14.5))
    ...
    parser.add_argument("--views-per-batch", type=int, default=1, help="Max views to sample per batch.")

    args = parser.parse_args()

    cfg = MultiviewTrainingConfig(
